...
int main(int argc, char **argv)
{
    ...
    // map the two devices
    offset = strtoul(argv[3], NULL, 16); // convert the offset to ul
    size = atoi(argv[4]) * PAGE_SIZE; // calculate total # of bytes
    ...
    // force a write of all user-space buffered data for stdout;
    // to make sure all data has been flushed to the device
    fflush(stdout);
...
static char *mapdev(const char *dev, unsigned long offset, unsigned long size)
{
    char *addr;
